...
class DoctorProfile(models.Model):
    ...
    specialty = models.CharField(
        max_length=100,
        help_text=_("Medical specialty (e.g., General Practice, Cardiology)")
    )
    ...
    clinic_name = models.CharField(
        max_length=200,
        help_text=_("Name of the clinic or hospital")
    )
    clinic_address = models.TextField(
        help_text=_("Physical address of the clinic")
    )
    consultation_fee = models.DecimalField(
        max_digits=10, 
        decimal_places=2,
        help_text=_("Standard consultation fee")
    )
    years_of_experience = models.PositiveIntegerField(
        default=0,
        help_text=_("Years of medical practice")
    )
...
class DoctorAvailability(models.Model):
    ...
    start_time = models.TimeField(
        help_text=_("Shift start time"),
        null=True,
        blank=True
    )
    end_time = models.TimeField(
        help_text=_("Shift end time"),
        null=True,
        blank=True
    )
    is_available = models.BooleanField(
        default=True,
        help_text=_("Whether the doctor is available on this day")
    )
