...
SYSTEM_INSTRUCTIONS = (
    ...
    "OUTPUT: Return ONLY compact JSON with keys: severity, summary, advice, red_flags, differential, rationale.\n"
    "severity MUST be one of: Mild, Moderate, Severe, Critical.\n"
    "CONTEXT: Use patient context carefully (age, weight, allergies, medical_history) to inform risk, red flags, and level of care.\n"
    "ALLERGIES: If allergies are present, note interactions or risks relevant to symptoms.\n"
    "COMORBIDITIES: From medical_history, factor chronic conditions that may increase severity.\n"
    "SAFETY: When in doubt, recommend contacting a healthcare professional. This is NOT medical advice.\n"
...
class GeminiClient:
        ...
        if self._api_variant is None and genai_legacy is not None:
            try:
                ...
                self._model_obj = genai_legacy.GenerativeModel(
                    model,
                    system_instruction=(
                        "You are a careful, conservative medical triage assistant.\n"
                        "Respond ONLY in compact JSON with keys: severity, summary, advice, red_flags, differential, rationale.\n"
                        "severity must be one of: Mild, Moderate, Severe, Critical.\n"
                        "Keep advice general and instruct patients to seek professional care for concerning symptoms.\n"
                        "This is NOT medical advice or diagnosis. Be concise."
                    ),
                )
    ...
    def generate_triage(self, symptoms_text: str, patient_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Returns a dict; if API missing/unavailable, raises RuntimeError.
        """
        if not self.enabled:
            raise RuntimeError("Gemini not configured. Set GEMINI_API_KEY and install google-genai / google-generativeai.")
        ...
        try:
            ...
            return {
                ...
                "advice": data.get("advice", "Consider contacting a healthcare professional for guidance."),
        ...
        for attempt in range(2):
            try:
                ...
                if self._api_variant == "new" and self._client is not None:
                    ...
                    for model_name in [self.model] + FALLBACK_MODELS:
                        try:
                            ...
                            resp = self._client.models.generate_content(model=model_name, contents=contents)
                            text = getattr(resp, "text", "") or ""
                            if not text:
                                try:
                                    parts = getattr(resp, "candidates", [])[0].content.parts  # type: ignore
                ...
                # Legacy path with model fallbacks
                    ...
                    for model_name in [self.model] + FALLBACK_MODELS:
                        try:
                            if model_obj is None or getattr(model_obj, "model_name", None) != model_name:
                                model_obj = genai_legacy.GenerativeModel(model_name, system_instruction=SYSTEM_INSTRUCTIONS)
    ...
    def _parse_or_repair(self, raw_text: str, original_prompt: str) -> dict:
        ...
        repair_prompt = (
            ...
            "If prior output was malformed, rewrite it as valid JSON with the exact required keys.\n\n"
            "PRIOR_REQUEST:\n"
            f"{original_prompt}\n"
        )
        try:
    ...
    def _build_prompt(self, symptoms_text: str, patient_context: Optional[Dict[str, Any]] = None) -> str:
